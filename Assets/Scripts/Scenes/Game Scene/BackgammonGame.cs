...
public class BackgammonGame : MonoBehaviour
{
    ...
    /// <summary>
    /// Updates the user interaction with the dice.
    /// </summary>
    ...
    private IEnumerator StartTurn()
    {
        ...
        {
            ShowAlert(string.Format("{0} is closed off and must yield this turn.", CurrentPlayer.Name));
            yield return Wait(3f);
            EndTurn();
        }
        else
        {
            ShowAlert(string.Format("{0}'s turn begin.", CurrentPlayer.Name));
    ...
    private void SelectDice()
    {
        ...
        if (Physics.Raycast(Camera.main.ScreenPointToRay(Input.mousePosition), out hit, 1000, LayerMask.GetMask("Dice")))
        {
            ...
            else if (hit.collider.tag == "Doubling Cube")
            {
                if (IsAIGame)
                {
                    ShowAlert("The doubling cube is disabled when playing against AI.");
                }
                else
                {
    ...
    /// <summary>
    /// <summary>
    /// Rolls the dice with random outcome.
    /// </summary>
    ...
    {
        StartCoroutine(RollDice(roll1, roll2, 1.5f));
    }
    ...
    public void OfferDoublingCubePreview()
    {
        ...
        {
            ShowAlert("You cannot offer the doubling cube in a Crawford game.");
            return;
        }
        ...
        {
            ShowAlert("You must control the doubling cube to offer it.");
        }
    }
    ...
    public void DeclineDoublingCube()
    {
        ...
        ShowAlert(string.Format("{0} forfeits this game.", GetOpponent(CurrentPlayer).Name));
    ...
    private bool CheckForWin(Player player)
    {
        if (Board.IsComplete(player))
        {
            ...
            {
                ShowAlert(string.Format("{0} wins a backgammon!", player.Name));
                Stakes *= 3;
            }
            else if (!Board.HasBorneOff(opponent))
            {
                ShowAlert(string.Format("{0} wins a gammon!", player.Name));
                Stakes *= 2;
            }
            else
            {
                ShowAlert(string.Format("{0} wins this game!", player.Name));
            }
